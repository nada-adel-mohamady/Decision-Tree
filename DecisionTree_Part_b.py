...
"""
 get the unique values of attributes of the dataset 
"""
...
"""
Functionality: create dictionary weights that contains each value of attributes in
data set with correspoding the initial weight.
Input: csv's filename of the training dataset
Output: return weight dictionary, its key is values of attribute and its value is the initiali weight of that  value which = 0
"""
def get_weights(filename):
    ...
    return myWeights
"""
**Functionality: this function creats list of dictionaries of test examples

**Input: csv'f filename of test set.

**Output: return the list of dictionaries of test examples, and the actual labels of
the test examples.
"""
...
"""
findMaxGain: this function calculate the information gain for each attribute
and return the attribute with the smallest information gain to get removed from 
the attributes that we will split based on them.
"""
    ...
    for j in columns:
        ...
        for i in rows:
            ...
            if key!=key:
                ...
                nn+=1
        gain = entropy
        ##inintilize frequency dictionary
        for key in mydict:
            N=0
            frequent_labels={}
            for label in labels:
            ...
            for k in rows:
                ...
                if miss:
                    N+=1
                    frequent_labels[data[k][-1]]+=mydict[key]/nn
                elif data[k][j] == key:
                    N+=1
...
"""handle missing values in test examples"""        
